        ...
        async with websockets.connect(url) as client:
            while True:
                while not position:
                    ...
                    diff0 = round(((float(data['k']['h']) - float(data['k']['l'])) * 100 / float(data['k']['h'])), 3)
                    if data['k']['x']:
                        ...
                        self.q = statistics.mean(list_vol) * 3.5
                    ...
                    if self.q < self.vol0 < float(data['k']['Q']) and self.q_max < float(data['k']['Q']) and self.average_diff > 0.15 and diff0 > 0.15 and rsi < 40:
                        ...
                        logger.info(f'{datetime.now()}, {self.pair} цена = {data["k"]["c"]}, объём {self.q} < {self.vol0} < {float(data["k"]["Q"])} ({round(float(data["k"]["Q"]) / self.vol0, 3)}), average_diff {round(self.average_diff, 2)}, diff0 {diff0}, rsi = {round(rsi, 2)}')
                        position = True
                        flag_average = False
                while position:
                    data = json.loads(await client.recv())
                    ...
                    if price_stop < float(data['k']['c']) <= price_average and not flag_average:
                        ...
                        logger.info(self.pair, 'AVERAGE', data['k']['c'], datetime.now())
                        price_take = (float(data['k']['c']) + price_buy) / 2 * 1.0045
                        flag_average = True
                    if float(data['k']['c']) <= price_stop:
                        sell_order_average(self.pair, self.depo, price_buy)
                        logger.info(self.pair, 'STOP_LOSS', data['k']['c'], datetime.now())
                        position = False
                        self.q = 1000000000
