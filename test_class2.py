        ...
        async with websockets.connect(url) as client:
            while True:
                while not position:
                    ...
                    if self.vol0 > self.q and average_diff > 0.15:
                        price_buy = float(data['k']['c'])
                        price_take = price_buy * 1.01
                        price_stop = price_buy * 0.99
                        print(f'{event_time}, {self.pair} цена = {data["k"]["c"]}, объём {self.vol0} > {self.q}, average_diff {average_diff}')
                        position = True
                while position:
                    data = json.loads(await client.recv())
