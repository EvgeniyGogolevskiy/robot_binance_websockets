            ...
            while True:
                while not position:
                    ...
                    if data['k']['x']:
                        ...
                        self.q = statistics.mean(list_vol) * 3
                    ...
                    if self.q < float(data['k']['Q']) * 2 < self.vol0 and self.q_max < self.vol0 and self.average_diff > 0.15 and rsi < 10:
                        price_buy = float(data['k']['c'])
                        price_take = price_buy * 1.005
                        price_stop = price_buy * 0.995
                        buy_order(self.pair, self.depo, price_buy)
                        print(f'{event_time}, {self.pair} цена = {data["k"]["c"]}, объём {self.q} < {float(data["k"]["Q"])} < {self.vol0}  ({round(self.vol0/float(data["k"]["Q"]),3)}), average_diff {round(self.average_diff,2)}, rsi = {round(rsi,2)}')
                        position = True
                while position:
                    data = json.loads(await client.recv())
                    if data['k']['x']:
...
if __name__ == '__main__':
    ...
    pairs = all_pairs.pair.dropna().unique()[10:35]
    try:
        for pair in pairs:
